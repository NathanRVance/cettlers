...
static int* roadlength_prepare(int roads[], int edges, int player)
{
 ...
 for(i = 0; ; i++) {
  for(j = 0; j < edges*2; j++) {
   if((i == 0 && map[i] > roads[j]) || (i != 0 && map[i-1] < roads[j] && map[i] > roads[j]))
    map[i] = roads[j];
  }
  if(map[i] == INF) {
   for(; i < edges*2+1; i++)
 ...
 for(i = 0; i < edges*2; i++) {
  for(j = 0; j < 3; j++) {
   if(graph[get_key(map, edges*2, roads[i])*3+j] == INF) {
    graph[get_key(map, edges*2, roads[i])*3+j] = get_key(map, edges*2, roads[i + (i%2) * -2 + 1]);
    break;
   }
  }
 }
 for(i = 0; map[i] != 0; i++) {
  for(j = 1; j < 4; j++) {
   if(data_atvertex((player+j)%4, map[i]) >= 1) deadends[i] = 1;
  }
 }
 return graph;
}
...
int dfs(int start, int graph[], int beento[], int parent)
{
 ...
 max = len = 0;
 for(k = 0; k < 3; k++) {
  if(graph[start*3+k] != INF && not_beento(beento, graph[start*3+k])) {
   for(i = 0; beento[i] != INF; i++);
   beento[i] = start;
   if(deadends[graph[start*3+k]]) len = 1;
   else len = dfs(graph[start*3+k], graph, beento, start);
   if(max < len) max = len;
   beento[i] = INF;
  }
  if(graph[start*3+k] != INF && ! not_beento(beento, graph[start*3+k]) && graph[start*3+k] != parent) {
   if(max == 0) max = 1;
  }
 }

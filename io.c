...
void io_printtrade() {
	mvprintw(27, 0,
			"          %3d %c  %3d %c   %3d %c   %3d %c   %3d %c   %s %c",
			trade[0], (trade[5] == 0) ? '*' : ' ', trade[1],
			(trade[5] == 1) ? '*' : ' ', trade[2], (trade[5] == 2) ? '*' : ' ',
			trade[3], (trade[5] == 3) ? '*' : ' ', trade[4],
			(trade[5] == 4) ? '*' : ' ', trade+6, (trade[5] == 5) ? '*' : ' ');
}

void io_printdat(int printsdat) {
	if (printsdat)
		printw("          %3d    %3d     %3d     %3d     %3d\n", dat[1][0],
				dat[1][1], dat[1][2], dat[1][3], dat[1][4]);
	int y = 1;
	int x = 54;
	int i;
	for (i = 0; i < 4; i++) {
...
void io_selectTrade(int accepted[3], int cursor, int player) {
	...
	int x = 10;
	mvprintw(y++, x, "##################################");
	mvprintw(y++, x, "# Player %d   Player %d   Player %d #",
			(player == 1) ? 2 : 1, (player <= 2) ? 3 : 2,
			(player <= 3) ? 4 : 3);
	mvprintw(y++, x, "# %s   %s   %s #", status[accepted[0]],
			status[accepted[1]], status[accepted[2]]);
	mvprintw(y++, x, "#    %c          %c          %c     #",
			(cursor == 0) ? '*' : ' ', (cursor == 1) ? '*' : ' ',
			(cursor == 2) ? '*' : ' ');
	mvprintw(y++, x, "##################################");
...
void io_printwin(int player, int cards) {
	mvprintw(11, 10, "#################################");
	mvprintw(12, 10, "# Player %d Won with %d VP card%s #", player + 1, cards,
			(cards == 1) ? "! " : "s!");
...
void io_printsetup(int cursor, int *set) {
	...
	mvprintw(y++, x, "#  %c  Player 1: %s                     #",
			(cursor == 0) ? '*' : ' ', (set[0] == 0) ? "human" : "ai   ");
	mvprintw(y++, x, "#  %c  Player 2: %s                     #",
			(cursor == 1) ? '*' : ' ', (set[1] == 0) ? "human" : "ai   ");
	mvprintw(y++, x, "#  %c  Player 3: %s                     #",
			(cursor == 2) ? '*' : ' ', (set[2] == 0) ? "human" : "ai   ");
	mvprintw(y++, x, "#  %c  Player 4: %s                     #",
			(cursor == 3) ? '*' : ' ', (set[3] == 0) ? "human" : "ai   ");
	mvprintw(y++, x, "#  %c  Map number distribution: %s  #",
			(cursor == 4) ? '*' : ' ',
			(set[4] == 0) ? "patterned" : "random   ");
	mvprintw(y++, x, "#  %c  Time between AI moves: %01.1fs         #",
			(cursor == 5) ? '*' : ' ', ((double) set[5]) / 10);
	mvprintw(y++, x, "#  %c  Animations: %s                #",
			(cursor == 6) ? '*' : ' ', (set[6] == 0) ? "disabled" : "enabled ");
	mvprintw(y++, x, "#                                         #");
...
void io_printrolled(void) {
	if (ROLLED)
		mvprintw(1, 0, "Rolled %d", ROLLED);
	if (stats_numrolls())
		mvprintw(2, 0, "Roll Count %d", stats_numrolls());
}

...
//Distribute numbers
	if (randomnums) {
		...
		int order[] = { 1, 3, 5, 10, 15, 18, 21, 19, 17, 12, 7, 4, 6, 8, 13, 16,
				14, 9, 11 };
		int desert;
...
//fix the location of the desert and the zero dice roll
	for (i = 0; map_getdat(i, 0) != 5; i++)
		; //i is the index of the desert
	for (j = 0; map_getdat(j, 1) != 0; j++)
		; //j is the index of zero
...
//First do resources
	i = 1;
	util_strreplace(map[map_getrowhex(i) * 2 + 5], "thing",
			map_getresname(map_getdat(i, 0)), 1);
	for (i = 3; i < 20; i++)
		util_strreplace(map[map_getrowhex(i) * 2 + 5], "thing",
				map_getresname(map_getdat(i, 0)), 1);
	i = 21;
...
//Next, roll numbers
	i = 1;
	util_strreplace(map[map_getrowhex(i) * 2 + 6], "##",
			map_itoa(map_getdat(i, 1)), 1);
	for (i = 3; i < 20; i++)
		util_strreplace(map[map_getrowhex(i) * 2 + 6], "##",
				map_itoa(map_getdat(i, 1)), 1);
	i = 21;
...
char* map_getresname(int res) {
	...
	case WOOD:
		return BRN "wood " NRM;
	case WHEAT:
		return YEL "wheat" NRM;
	case SHEEP:
		return GRN "sheep" NRM;
	case BRICK:
		return RED "brick" NRM;
	case STONE:
		return "stone";
	case DESERT:
		return "     ";
...
int map_getroadnum(int pos, int direction) {
	int ans = pos % 6;
	if (pos <= 16 && direction == 0)
		ans--;
	if (pos <= 10)
		ans--;
	if (pos <= 3)
		ans--;
	if (pos >= 49 && direction == 1)
		ans--;
	if (pos >= 55)
		ans--;
	if (pos >= 62)

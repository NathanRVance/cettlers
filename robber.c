...
static int getoffset(int hex) {
	...
	if (hex % 5 == 0 || hex % 5 == 3)
		return offset;
	if (hex % 5 == 2 && map_getdat(hex - 2, 1) == map_getdat(hex, 1))
		offset++;
	if (map_getdat(hex - 1, 1) == map_getdat(hex, 1))
		offset++;
	return offset;
}
...
void robber_erase(int hex) {
	...
	util_strreplace(map[row], "robber",
			cat(cat(" ", map_itoa(map_getdat(hex, 1))), "   "), 1);
}

void robber_place(int hex) {
	map_setdat(hex, 2, 1);
	int row = map_getrowhex(hex) * 2 + 6;
	util_strreplace(map[row],
			cat(cat(cat(ROBBER, " "), map_itoa(map_getdat(hex, 1))), "   "),
			ROBBER "robber", getoffset(hex));
}
...
void robber_stealroutine(int player, int hex) {
	...
	i = -1;
	if (robber_atvert(player, vert) >= 0)
		verts[++i] = vert;
	if (robber_atvert(player, vert + 1) >= 0)
		verts[++i] = vert + 1;
	if (robber_atvert(player, vert + 6) >= 0)
		verts[++i] = vert + 6;
	if (robber_atvert(player, vert + 7) >= 0)
		verts[++i] = vert + 7;
	if (robber_atvert(player, vert + 12) >= 0)
		verts[++i] = vert + 12;
	if (robber_atvert(player, vert + 13) >= 0)
		verts[++i] = vert + 13;
	if (i == -1)
		return;
	int v1, v2, v3;
	...
	if (i >= 1)
		v2 = robber_atvert(player, verts[1]);
	if (i == 2)
		v3 = robber_atvert(player, verts[2]);
	...
	if (v1 != v2 || v1 != v3 || v2 != v3) {
		...
		while (rob) {
			...
			case UP:
				...
				break;
			case DOWN:
				marker_setposition(vert = verts[i]);
				break;
			case LEFT:
				marker_setposition(vert = verts[i / 2]);
				break;
			case RIGHT:
				marker_setposition(vert = verts[(i + 1) / 2]);
				break;
			case ENTER:
			case 'r':
				rob = 0;
...
void robber_discardhandler(int player, int num) {
	if (data_isai(player))
		ai_discard(player, num);
	else {
		trade_routine_other(player, num * -1, 2);
	}
}
